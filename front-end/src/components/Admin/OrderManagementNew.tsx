...
type SortField = "no" | "id" | "date" | "total" | "customer";
...
function OrderStatusBadge({ status }: { status: OrderStatus }) {
  ...
  const displayStatus = status === "PENDING_PAYMENT" ? "PENDING PAYMENT" : status;

  return (
    <span
      className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${statusColors[status] || "bg-gray-100 text-gray-800 border-gray-200"
        }`}
    >
      {displayStatus}
    </span>

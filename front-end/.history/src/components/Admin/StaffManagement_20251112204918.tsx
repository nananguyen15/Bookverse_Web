...
export function StaffManagement() {
  ...
  const handleCreate = async () => {
    try {
      ...
      // Backend creates with CUSTOMER role by default, need to change to STAFF
      const createData = {
      ...
      // Step 1: Create user (will have CUSTOMER role by default)
      const newUser = await usersApi.create(createData);
      
      // Step 2: Change role from CUSTOMER to STAFF
      await usersApi.changeRole(newUser.id);
      
      // Step 3: If user uploaded a file, update with the file after creation
      if (imageFile) {

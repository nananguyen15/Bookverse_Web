...
export function CustomerManagement() {
  ...
  const handleCreate = async () => {
    try {
      ...
      // Prepare create data - Backend accepts JSON only (UserCreationRequest)
      const createData = {
        ...
        image: formData.image && formData.image.trim() ? formData.image.trim() : undefined, // Only URL/path, no file upload in create
        active: true,
      };

      // Create user (role will be CUSTOMER by default)
      const newUser = await usersApi.create(createData);
      
      // If user uploaded a file, update with the file after creation
      if (imageFile) {
        await usersApi.update(newUser.id, { imageFile });
      }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      // ALWAYS include at least name (required for multipart to work properly)
      updateData.name = formData.name && formData.name.trim() ? formData.name.trim() : selectedUser.name;

      // Only include other fields if they have NEW values
      if (formData.phone && formData.phone.trim() && formData.phone !== selectedUser.phone) {
        updateData.phone = formData.phone.trim();
      }

      if (formData.address && formData.address.trim() && formData.address !== selectedUser.address) {
      ...
      alert(`Failed to update customer: ${errorMsg}`);
    }
  };
            ...
            {paginatedCustomers.length === 0 ? (
              ...
              paginatedCustomers.map((customer, index) => (
                <tr key={customer.id} className="transition-colors hover:bg-beige-50">
                  ...
                  <TableCell>
                    <TableCellText variant="secondary">{customer.id}</TableCellText>
                  </TableCell>
                  <TableCell>

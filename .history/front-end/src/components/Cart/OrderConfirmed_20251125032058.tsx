        ...
        try {
          ...
          if (vnp_ResponseCode === "00") {
            ...
            try {
              ...
              // We need to extract payment ID from vnp_TxnRef (this is the payment ID from backend)
              const paymentId = vnp_TxnRef ? parseInt(vnp_TxnRef) : null;

              console.log("💳 Extracted Payment ID from TxnRef:", paymentId);

              if (paymentId) {
                try {
                  // Step 6: Update payment status to SUCCESS
                  console.log("🔄 Updating payment status to SUCCESS...");
                  const updatedPayment = await paymentApi.markPaymentDone(paymentId);
                  console.log("✅ Payment updated:", updatedPayment);

                  const orderId = updatedPayment.orderId;
                  ...
                  // Step 7: Redirect to order-confirmed page with orderId
                  if (orderId) {
                  ...
                  // Even if update fails, show success because VNPay confirmed payment
                  if (isMounted) {
                    setPaymentSuccess(true);
                    setPaymentProcessed(true);
                    setHasProcessed(true);
                ...
                console.error("❌ Could not extract payment ID from TxnRef");

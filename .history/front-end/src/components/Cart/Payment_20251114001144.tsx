...
import { usersApi, paymentApi } from "../../api";
...
export function Payment() {
  ...
  const onSubmit = async (data: ShippingFormValues) => {
    ...
    const newOrder = {
      orderId,
      items: cartDetails.selectedItems,
      summary: { ...cartDetails },
      shippingInfo: data,
      address: fullAddress,
      paymentMethod,
    ...
    // Save order to localStorage (temporary - should use API)
    ...
    if (saveToProfile && user) {
      try {
        // Cast user to any because the User type may not declare an 'id' property
        await usersApi.update((user as any).id, {
          name: data.fullName,
          phone: data.phone,
          address: fullAddress,
        });
    ...
    if (paymentMethod === "vnpay") {
      try {
        ...
        // Backend returns 'URL' field (uppercase), not 'paymentUrl'
      ...
      // COD payment - direct to confirmation
      clearCart();

        ...
        try {
          ...
          if (vnp_ResponseCode === "00") {
            ...
            try {
              ...
              // Backend returns success but doesn't update payment status automatically
              // Since backend doesn't properly map TxnRef to payment ID, we need to find the payment by amount
              const vnpayAmountInVND = parseInt(vnp_Amount || "0"); // VNPay amount in VND cents (multiply by 100)
              const expectedAmountInVND = vnpayAmountInVND / 100; // Convert to actual VND
              
              console.log("💰 VNPay Amount (VND):", expectedAmountInVND);
              ...
              try {
                ...
                // Find order with matching amount and PENDING payment
                const matchingOrder = orders.find((order: any) => {
                  const orderAmountInVND = Math.round(order.totalAmount * 25000);
                  return order.payment?.status === "PENDING" && 
                         order.payment?.method === "VNPAY" &&
                         Math.abs(orderAmountInVND - expectedAmountInVND) < 100; // Allow small difference
                });
                ...
                if (matchingOrder && matchingOrder.payment) {
                  ...
                  console.error("❌ Could not find matching order with pending payment");
                  console.log("Expected amount (VND):", expectedAmountInVND);

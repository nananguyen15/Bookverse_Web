  ...
  const handleVNPayPayment = async () => {
    ...
    try {
      const { paymentUrl } = await paymentApi.createPayment({
        ...
        orderInfo: `BookVerse Order - ${selectedItems.length} items`,
      });
              ...
              <div className="space-y-4 lg:col-span-2">
                ...
                {itemsWithDetails.map((item) => {
                  ...
                  return (
                    <div
                      key={key}
                      className={`grid grid-cols-[auto,1fr,200px] gap-4 p-4 transition-all duration-300 bg-white border shadow-sm rounded-xl hover:shadow-md ${item.selected ? "border-beige-300" : "border-beige-100"
                        }`}
                    >
                      ...
                      <div className="flex items-start gap-3">
                        <input
                          ...
                          className="w-5 h-5 mt-1 border-2 rounded cursor-pointer text-beige-700 border-beige-300 focus:ring-2 focus:ring-beige-500"
                        ...
                        <div className="relative group">
                          <img
                            src={item.image}
                            alt={item.title}
                            className="object-cover w-24 h-32 transition-transform duration-300 rounded-lg group-hover:scale-105"
                          />
                        </div>
                      </div>

                      {/* Column 2: Product Info + Quantity */}
                      <div className="flex flex-col justify-between">
                        <div>
                          <h2 className="mb-1 text-base font-bold text-beige-900 line-clamp-2">
                            {item.title}
                          </h2>
                          {item.categoryName && (
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          <div className="flex items-center gap-2">
                            <button
                              ...
                              className="flex items-center justify-center w-8 h-8 transition-all duration-200 border rounded-lg border-beige-300 text-beige-700 hover:bg-beige-50 hover:border-beige-400 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed"
                            >
                              -
                            </button>
                            <input
                              ...
                              className="w-16 px-2 py-1 text-center border rounded-lg border-beige-300 focus:ring-2 focus:ring-beige-500 focus:border-beige-500"
                            />
                            <button
                              onClick={() =>
                                updateQuantity(
                      ...
                      {/* Column 3: Total Price + Remove Icon */}

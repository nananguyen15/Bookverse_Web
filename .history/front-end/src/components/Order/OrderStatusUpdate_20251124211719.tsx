...
export function OrderStatusUpdate({
  ...
  const [selectedStatus, setSelectedStatus] = useState<OrderStatus | "">(""); const [isLoading, setIsLoading] = useState(false);
    ...
    return (
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
        <p className="text-sm text-gray-600">
          Đơn hàng ở trạng thái cuối cùng, không thể thay đổi.
        </p>
      </div>
    );
  }
      ...
      <div className="bg-white border border-gray-200 rounded-lg p-4">
        ...
        <div className="mt-3 bg-blue-50 border-l-4 border-blue-400 p-3">
          <p className="text-xs text-blue-700">
            <strong>Lưu ý:</strong> Chỉ có thể chuyển trạng thái theo đúng quy
            trình: PENDING → CONFIRMED → PROCESSING → DELIVERING → DELIVERED
          </p>
        </div>
      </div>
    </div>

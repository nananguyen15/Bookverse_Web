...
export function StaffProfile() {
  ...
  useEffect(() => {
    if (user?.username) {
      ...
      const staffUser = users.find((u: { username: string }) => u.username === user.username);
  ...
  const onSubmit = (data: ProfileFormValues) => {
    ...
    const users = JSON.parse(localStorage.getItem("users") || "[]");
    const updatedUsers = users.map((u: { username: string; fullName?: string; phone?: string; email?: string; avatarUrl?: string }) => {

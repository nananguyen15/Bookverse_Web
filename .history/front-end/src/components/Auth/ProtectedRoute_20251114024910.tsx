...
function UnauthorizedAccess({
  ...
  useEffect(() => {
    ...
    // Navigate back immediately after alert
  ...
  return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="text-center">
        <div className="inline-block w-8 h-8 border-4 rounded-full border-t-transparent animate-spin border-beige-700"></div>
        <p className="mt-4 text-beige-600">Redirecting...</p>
      </div>
    </div>
  );
}
...
export function ProtectedRoute({
  ...
  if (allowedRoles && allowedRoles.length > 0) {
    if (!userRole || !allowedRoles.includes(userRole)) {
      ...
      return (
        <UnauthorizedAccess userRole={userRole} allowedRoles={allowedRoles} />
      );
    }
  }

...
export function AllProducts({ defaultType }: AllProductsProps) {
  ...
  useEffect(() => {
    const fetchBooks = async () => {
      try {
        ...
        // IMPORTANT: Filter to ensure only active books (in case backend returns inactive)
        const activeBooksOnly = booksWithNames.filter(book => book.active === true);
        
        console.log('Total books from API:', booksWithNames.length);
        console.log('Active books only:', activeBooksOnly.length);
        console.log('Filtered out inactive:', booksWithNames.length - activeBooksOnly.length);

        setBooks(activeBooksOnly);
  ...
  // For /books page: Only show ACTIVE books
  // Inactive books should only appear in search results with restrictions
  const allProducts = useMemo(
    () => [
      ...
      // Don't include inactive books on /books page
    ],
    [books]
  );

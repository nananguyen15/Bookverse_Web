    ...
    <div className="p-6 bg-white border rounded-lg shadow-sm border-beige-100 hover:shadow-md transition-shadow">
      <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
        ...
        <div className="flex items-center flex-wrap gap-2">
          <Link
            to={`/order/${order.id}`}
            className="px-3 py-1.5 text-xs font-semibold text-white rounded-md bg-beige-700 hover:bg-beige-800 transition-colors"
          >
            View Details
          </Link>
          {isPaymentPending && (
            <button
              ...
              className="px-3 py-1.5 text-xs font-semibold text-white bg-green-600 rounded-md hover:bg-green-700 transition-colors"
            >
              Complete Payment
            </button>
          )}
          {canCancelOrder(order.status) && !isPaymentPending && (
            <button
              ...
              className="px-3 py-1.5 text-xs font-semibold border rounded-md text-red-600 bg-white border-red-300 hover:bg-red-50 transition-colors"
            >
              Cancel Order
            </button>
          )}
          {canChangeAddress(order.status) && !isPaymentPending && (
            <button
              ...
              className="px-3 py-1.5 text-xs font-semibold border rounded-md text-beige-700 bg-white border-beige-300 hover:bg-beige-50 transition-colors"
            >
              Change Address
            </button>
          )}
          {order.status === "DELIVERED" && (
            <>
              <button
                ...
                className="px-3 py-1.5 text-xs font-semibold text-white rounded-md bg-beige-600 hover:bg-beige-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {buyingAgain ? "Adding..." : "Buy Again"}
              </button>
              <button
                onClick={handleReview}
                className="flex items-center px-3 py-1.5 text-xs font-semibold text-white bg-yellow-500 rounded-md hover:bg-yellow-600 transition-colors"
              >

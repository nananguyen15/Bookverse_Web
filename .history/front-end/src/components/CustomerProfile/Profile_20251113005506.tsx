  ...
  useEffect(() => {
    const loadProfile = async () => {
      try {
        ...
        if (userInfo.image) {
          setAvatarPreview(transformImageUrl(userInfo.image) || "");
        }
      } catch (error) {
    ...
    try {
      ...
      if (avatarChanged) {
        if (useUrlUpload && imageUrlInput.trim()) {
          ...
          // Use file upload
          console.log("✅ Using image file:", avatarFile.name, `(${avatarFile.size} bytes)`);
      ...
      if (updatedUser.image) {
        setAvatarPreview(transformImageUrl(updatedUser.image) || "");
      }

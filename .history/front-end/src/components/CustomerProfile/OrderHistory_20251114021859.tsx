      ...
      {statuses.map((s, index) => (
        <div key={s} className="flex items-center flex-1">
          <div className="flex flex-col items-center">
            <div
              className={`w-6 h-6 rounded-full flex items-center justify-center ${index <= currentIndex ? "bg-beige-700" : "bg-gray-300"
                }`}
            >
              <FaCheckCircle className="text-white" size={14} />
            </div>
            <p
              className={`mt-1 text-xs ${index <= currentIndex ? "text-beige-900" : "text-gray-500"
                }`}
            >
              {s}
            </p>
          ...
          {index < statuses.length - 1 && (
            <div
              className={`flex-1 h-1 mx-2 ${index < currentIndex ? "bg-beige-700" : "bg-gray-300"
                }`}
            ></div>
          )}
        </div>
    ...
    <div className="p-6 bg-white border rounded-lg shadow-sm border-beige-200">
      <div className="flex flex-wrap items-center justify-between gap-4 mb-4">
        <div>
          ...
          <p className="text-lg font-semibold text-beige-700">

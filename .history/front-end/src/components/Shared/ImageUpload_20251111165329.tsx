 ...
 * Automatically uploads files to backend server
 */
export function ImageUpload({
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      // Create preview FIRST (for immediate visual feedback)
      ...
      const dbPath = await uploadApi.uploadImage(file, folder);
      
      console.log(`✅ Upload successful!`);
      console.log(`💾 DB path: ${dbPath}`);
      
      // Save DB path to form state
      onChange(dbPath);
    ...
    <div className="space-y-3">
        ...
        <div className="relative inline-block">
          ...
          <div className="mt-2 text-xs text-gray-600 break-all max-w-xs">
            {(() => {
              ...
              // If it's a Base64 string, show truncated version
              if (displayValue.startsWith('data:image')) {
                return `${displayValue.substring(0, 50)}... [Base64 Image Data]`;
              }
              return displayValue;
            })()}
          </div>
      ...
      <p className="text-xs text-gray-500">
        {uploadMode === "file"
          ? uploading 
            ? "⏳ Uploading to server..."
            : "✅ File will be uploaded to server automatically"

...
export function StatisticDashboard() {
  ...
  const [timeRange, setTimeRange] = useState<"1W" | "1M" | "3M" | "6M" | "1Y">(
    "1M"
  );
  ...
  const loadStatistics = async () => {
    try {
      ...
      // Mock sales by month data (backend doesn't provide this yet)
      ...
      setStatistics({
        ...
        averageOrderValue,
        salesGrowth: 12.5, // Mock data - backend doesn't provide growth stats yet
  ...
  const exportReport = () => {
    ...
    link.href = url;
    link.download = `bookverse-report-${new Date().toISOString().split("T")[0]
      }.json`;
    link.click();
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white p-6 rounded-lg shadow-sm border border-beige-200">
          <div className="flex items-center justify-between mb-4">
            ...
            <span
              className={`flex items-center gap-1 text-sm font-medium ${statistics.salesGrowth >= 0 ? "text-green-600" : "text-red-600"
                }`}
            >
          ...
          <p className="text-beige-500 text-sm mt-2">
            +{statistics.salesGrowth}% vs last month
          </p>
        </div>
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-beige-200">
          <div className="flex items-center justify-between mb-4">
            ...
            <span
              className={`flex items-center gap-1 text-sm font-medium ${statistics.ordersGrowth >= 0 ? "text-green-600" : "text-red-600"
                }`}
            >
          ...
          <p className="text-beige-500 text-sm mt-2">
            +{statistics.ordersGrowth}% vs last month
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {/* Sales Revenue Chart */}
        <div className="bg-white p-6 rounded-lg shadow-sm border border-beige-200">
          <h3 className="text-xl font-bold text-beige-900 mb-6">
            Sales Revenue
          </h3>
            ...
            {statistics.salesByMonth.map((data, index) => (
              <div key={index}>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-beige-600 text-sm font-medium">
                    {data.month}
                  </span>
                  <span className="text-beige-900 font-bold">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-beige-200">
                <th className="text-left py-3 px-4 text-beige-600 font-semibold">
                  #
                </th>
                <th className="text-left py-3 px-4 text-beige-600 font-semibold">
                  Product Name
                </th>
                <th className="text-right py-3 px-4 text-beige-600 font-semibold">
                  Sales
                </th>
                <th className="text-right py-3 px-4 text-beige-600 font-semibold">
                  Quantity
              ...
              {statistics.topProducts.length > 0 ? (
                statistics.topProducts.map((product, index) => (
                  ...
                  >
                    ...
                    <td className="py-4 px-4 text-beige-900 font-medium">
                      {product.name}
                    </td>
                    <td className="py-4 px-4 text-right text-beige-700">
                      {product.sales}
                    </td>
                    <td className="py-4 px-4 text-right text-beige-700">
                      {product.quantity}

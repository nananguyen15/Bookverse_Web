...
export function SubCategoryManagement() {
  ...
  // Form data
  const [subFormData, setSubFormData] = useState<SubCategoryCreateRequest>({
    supCategoryId: 0,
    name: "",
    description: "",
    active: true,
  ...
  const handleCreateSub = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubFormData({ supCategoryId: 0, name: "", description: "", active: true });
      loadAllCategories();
    } catch (error) {
  ...
  const handleUpdateSub = async (e: React.FormEvent) => {
    ...
    try {
      await categoriesApi.sub.update(selectedSub.id, subFormData as SubCategoryUpdateRequest);

...
// Extend CategoryWithSubs to support both camelCase and lowercase property names from API
interface CategoryWithSubcategories {
  id: number;
  name: string;
  active: boolean;
  subcategories?: SubCategory[]; // API returns lowercase
  subCategories?: SubCategory[]; // Type definition uses camelCase
}
...
export function Promotion() {
  ...
  const getCategoryName = (categoryId: string) => {
    ...
    for (const mainCat of categories) {
      if ('subcategories' in mainCat && Array.isArray(mainCat.subcategories)) {
        const subCat = mainCat.subcategories.find(
          (sc: { id: number; name: string }) => sc.id.toString() === categoryId
        );
                      ...
                      categories.map((mainCat) => (
                        <div key={mainCat.id} className="ml-4 space-y-1">
                          ...
                          {mainCat.subcategories &&
                            mainCat.subcategories.map((subCat: SubCategory) => (
                              <label
                                key={subCat.id}

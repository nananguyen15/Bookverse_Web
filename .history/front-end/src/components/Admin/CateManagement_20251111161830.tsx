  ...
  const handleToggleSupStatus = async (sup: SupCategory) => {
    try {
      if (sup.active) {
        ...
        // FRONTEND CASCADE: Deactivate sup category first
        await categoriesApi.sup.deactivate(sup.id);
        
        // Get all sub categories of this sup category
        const subCategories = subCategoriesList.filter(sub => sub.supCategoryId === sup.id);
        
        // Deactivate each sub category (which will also deactivate books if backend supports it)
        ...
        alert(`✅ Đã ẩn danh mục cha "${sup.name}" và ${subCategories.length} danh mục con!`);
      } else {
        ...
        alert(`✅ Đã kích hoạt lại danh mục cha "${sup.name}" và ${subCategories.length} danh mục con!\n(Sách vẫn cần kích hoạt thủ công)`);
      }
      loadAllCategories();
    } catch (error) {

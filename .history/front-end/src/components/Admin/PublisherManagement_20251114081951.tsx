...
export function PublisherManagement() {
  ...
  const handleCreate = async () => {
    try {
      ...
      alert(`Unable to create publisher: ${err.response?.data?.message || err.message}`);
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      console.error("Error response:", err.response?.data);
      alert(`Unable to update publisher: ${err.response?.data?.message || err.message}`);
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold text-beige-900">Publisher Management</h1>

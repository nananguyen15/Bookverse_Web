...
export function CateManagement() {
  ...
  const handleToggleSupStatus = async (sup: SupCategory) => {
    try {
      if (sup.active) {
        // Confirm before deactivating (cascade effect warning)
        const confirmed = window.confirm(
          `⚠️ Cảnh báo:\n\n` +
          `Khi ẩn danh mục cha "${sup.name}", hệ thống sẽ TỰ ĐỘNG:\n` +
          `• Ẩn TẤT CẢ danh mục con thuộc danh mục này\n` +
          `• Ẩn TẤT CẢ sách thuộc các danh mục con đó\n\n` +
          `Bạn có chắc chắn muốn tiếp tục?`
        );
        
        if (!confirmed) return;
        
        // Get all sub categories of this sup category BEFORE deactivating
        const subCategories = subCategoriesList.filter(sub => sub.supCategoryId === sup.id);
        
        // FRONTEND CASCADE DEACTIVATION
        // Step 1: Deactivate all sub categories (backend should cascade to books if code is compiled)
        let deactivatedCount = 0;
        for (const subCat of subCategories) {
          try {
            await categoriesApi.sub.deactivate(subCat.id);
            deactivatedCount++;
          } catch (error) {
            console.error(`Error deactivating sub category ${subCat.id}:`, error);
          }
        }
        
        // Step 2: Deactivate sup category
        await categoriesApi.sup.deactivate(sup.id);
        
        alert(`✅ Đã ẩn:\n• 1 danh mục cha: "${sup.name}"\n• ${deactivatedCount} danh mục con\n• Các sách liên quan (tự động)`);
      } else {
        // Get all sub categories of this sup category BEFORE activating
        const subCategories = subCategoriesList.filter(sub => sub.supCategoryId === sup.id);
        
        // Step 1: Activate sup category first
        await categoriesApi.sup.activate(sup.id);
        
        // Step 2: Reactivate all sub categories
        let activatedCount = 0;
        for (const subCat of subCategories) {
          try {
            await categoriesApi.sub.activate(subCat.id);
            activatedCount++;
          } catch (error) {
            console.error(`Error activating sub category ${subCat.id}:`, error);
          }
        }
        
        alert(`✅ Đã kích hoạt lại:\n• 1 danh mục cha: "${sup.name}"\n• ${activatedCount} danh mục con\n⚠️ Lưu ý: Sách vẫn cần kích hoạt thủ công`);
      }
      loadAllCategories();
    } catch (error) {
  ...
  const handleToggleSubStatus = async (sub: SubCategory) => {
    try {
      if (sub.active) {
        ...
        const confirmed = window.confirm(
          `⚠️ Cảnh báo:\n\n` +
          `Khi ẩn danh mục con "${sub.name}", hệ thống sẽ TỰ ĐỘNG:\n` +
          `• Ẩn TẤT CẢ sách thuộc danh mục này\n\n` +
          `Bạn có chắc chắn muốn tiếp tục?`
        );
        ...
        alert("✅ Đã ẩn danh mục con và tất cả sách liên quan!");
      } else {
        await categoriesApi.sub.activate(sub.id);
        alert("✅ Đã kích hoạt lại danh mục con!\n(Sách vẫn cần kích hoạt thủ công)");
      }
      loadAllCategories();
    } catch (error) {

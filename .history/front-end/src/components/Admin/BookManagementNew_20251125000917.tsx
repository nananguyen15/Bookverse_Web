...
export function BookManagementNew() {
  ...
  const handleCreate = async () => {
    try {
      ...
      // Auto-fill publishedDate with today's date if not provided
      const dataToSubmit = { ...formData };
      if (!dataToSubmit.publishedDate || !dataToSubmit.publishedDate.trim()) {
        const today = new Date().toISOString().split('T')[0];
        dataToSubmit.publishedDate = today;
        console.log("📅 Published date not provided, using today's date:", today);
      }

      console.log("📚 Creating book with data:", dataToSubmit);
      console.log("📷 Image file:", imageFile);

      await booksApi.create({ ...dataToSubmit, imageFile });

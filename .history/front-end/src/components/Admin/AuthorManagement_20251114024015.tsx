...
export function AuthorManagement() {
  ...
  const handleCreate = async () => {
    try {
      ...
      alert(`Unable to create author: ${errorResponse.response?.data?.message || errorResponse.message}`);
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const errorResponse = error as { response?: { data?: { message?: string; error?: string }; status?: number }; message?: string };
      console.error("❌ Error response:", errorResponse.response?.data);
      console.error("❌ Error status:", errorResponse.response?.status);
      console.error("❌ Full error:", error);

      const errorMsg = errorResponse.response?.data?.message || errorResponse.response?.data?.error || errorResponse.message;
  ...
  const handleToggleStatus = async (author: Author) => {
    try {
      if (author.active) {
        ...
        alert("Activated author!");
      }
      loadAuthors();
    } catch (error) {
      console.error("Error toggling status:", error);
      alert("Can not change author's status");
    }
  };
  ...
  return (
    <div className="p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-3xl font-bold text-beige-900">Author Management</h1>
          <p className="mt-1 text-sm text-beige-600">
            Total: {filteredAuthors.length} authors
          </p>
        </div>
        <button
            ...
            {paginatedAuthors.length === 0 ? (
              ...
              paginatedAuthors.map((author) => (
                ...
                >
                  ...
                  <TableCell>
                    <div className="max-w-xs truncate" title={author.biography || "No description"}>

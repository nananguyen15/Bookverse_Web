...
export const usersApi = {
  ...
  // PUT update user - JSON only (no file upload in update)
  update: async (
    userId: string,
    ...
    if (data.imageFile) {
      // If file upload needed, use FormData with multipart endpoint
      const formData = new FormData();
      ...
      // No file upload - use JSON endpoint (more reliable)
      ...
      if (data.image !== undefined) jsonData.image = data.image; // imageUrl

      const response = await apiClient.put<ApiResponse<User>>(
        `${USERS_ENDPOINT}/update-json/${userId}`,
        jsonData
      );
      return response.data.result;
    }

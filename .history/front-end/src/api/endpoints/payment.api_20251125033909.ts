...
export const paymentApi = {
   ...
   * @param data VNPay URL creation request (paymentId and amount)
  ...
  /**
   * Mark payment as refunded (REFUNDED)
   * PUT /api/payments/status-refunded/{paymentId}
   * Can only be called when payment status = REFUNDING
   * Used by staff/admin after processing refund
   * @param paymentId Payment ID
   * @returns Updated payment response
   */
  markPaymentRefunded: async (paymentId: number): Promise<PaymentResponse> => {
    const response = await apiClient.put<ApiResponse<PaymentResponse>>(
      `${PAYMENT_ENDPOINT}/status-refunded/${paymentId}`
    );
    return response.data.result;
  },

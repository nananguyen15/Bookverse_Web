...
const apiClient: AxiosInstance = axios.create({
  ...
  transformRequest: [
    (data, headers) => {
      // If data is FormData, don't transform and let browser handle Content-Type
      if (data instanceof FormData) {
        return data;
      }
      // For other data, use default transformation (JSON stringify)
...
apiClient.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    // Browser will automatically set multipart/form-data with boundary for FormData
    if (
      config.data &&
      Object.prototype.toString.call(config.data) === "[object FormData]"
    ) {
      console.log("📦 Detected FormData - removing Content-Type to let browser set it");
